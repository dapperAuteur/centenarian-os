...
export const HELP_ARTICLES: HelpArticle[] = [
...

  {
    ...
    content: `Teller is a bank account linking service that brings your bank transactions into CentenarianOS. Go to Dashboard → Finance → Accounts and click Connect Bank. Select your bank in Teller's window and authorize access. When you first connect, CentenarianOS imports the transaction history your bank makes available. After that, new transactions come in only when you press Sync: Sync next to an account updates every account from that bank login, and Sync All updates all of your linked banks. Nothing syncs automatically in the background. Each sync re-checks the 10 days before your previous sync, so a pending charge that posts with a new amount, date, or description (for example a restaurant tip) is updated, as long as you haven't edited that transaction. Each synced transaction includes date, amount, description, and merchant, and it gets the vendor's learned category if you set one with "Always". Unlinking an account (the unlink icon on the account) only stops CentenarianOS from syncing it; its existing synced transactions stay in your account. Unlinking does not disconnect your bank: the bank connection itself stays active with Teller. To fully disconnect your bank, tap the Help & Feedback (+) button in the bottom-right corner of any dashboard page, choose Feedback, and ask us to disconnect it. Teller supports most major US banks and credit unions.`,
  },
  {
    role: 'all',
