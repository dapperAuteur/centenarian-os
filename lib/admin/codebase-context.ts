...
4. **Planner** — Tasks, milestones, roadmaps, goals. One-field task capture (POST /api/tasks, offline-queued, idempotent client id) files goal-less tasks in an auto-created Inbox roadmap; system roadmaps (Inbox, Work.WitUS Sync) are flagged by roadmaps.system_kind (migration 199) and shown with an Auto badge. Weekly AI review via Gemini. Task location linking via saved contacts with sub-locations. Calendar import (.ics parser, pure TypeScript). Life retrospective AI analysis of calendar history patterns.
