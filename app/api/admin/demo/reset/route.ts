...
async function resetUser(supabase: SupabaseClient, userId: string, type: 'tutorial' | 'visitor') {
  for (const table of CLEAR_ORDER) {
    const { error } = await supabase.from(table).delete().eq('user_id', userId);
    if (error) throw new Error(`Failed to clear ${table} for ${type}: ${error.message}`);
  }
  if (type === 'tutorial') {
...
async function seedTutorial(supabase: SupabaseClient, userId: string) {
  // Accounts
...
// ─── VISITOR ACCOUNT ────────────────────────────────────────────────────────
// Richer data — gives free-class visitors a full picture of the app's capabilities.

async function seedVisitor(supabase: SupabaseClient, userId: string) {
  // Accounts
  const { data: accts, error: acctErr } = await supabase
    .from('financial_accounts')
    .insert([
  ...
  // Fuel logs (including road trip legs)
  if (crvId) {
    const { error: fuelErr } = await supabase.from('fuel_logs').insert([
  ...
  // Trips (mix of commutes, fitness rides, road trip)
  const { error: tripErr } = await supabase.from('trips').insert([
