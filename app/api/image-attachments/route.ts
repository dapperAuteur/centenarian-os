...
export async function POST(request: NextRequest) {
  ...
  const { entity_type, entity_id, image_url, image_public_id, label } = body;

  if (!entity_type || !entity_id || !image_url) {
    return NextResponse.json({ error: 'entity_type, entity_id, and image_url are required' }, { status: 400 });
  }
...
export async function DELETE(request: NextRequest) {
  ...
  if (attachment.image_public_id) {
    deleteCloudinaryAsset(attachment.image_public_id).catch(() => {});
  }
