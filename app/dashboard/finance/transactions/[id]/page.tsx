...
import {
  ...
  Loader2, ArrowDownLeft, ArrowUpRight, Building2, CreditCard, FileText, Landmark, Unlink,
} from 'lucide-react';
...
export default function TransactionDetailPage() {
  ...
  const [unmatchNotice, setUnmatchNotice] = useState<{ text: string; bankId: string | null } | null>(null);
  ...
  // Split a manual or scanned entry from the bank transaction a sync linked it to.
  const handleUnmatch = async () => {
    setActionLoading('unmatch');
    setUnmatchError(null);
    try {
      const res = await offlineFetch('/api/teller/unmatch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ transaction_id: id }),
      });
      if (isQueuedResponse(res)) {
        setUnmatchNotice({ text: "You're offline. The unmatch is queued and will run when you reconnect.", bankId: null });
        setConfirmUnmatch(false);
        return;
      }
      const data = await res.json().catch(() => null);
      if (!res.ok) {
        setUnmatchError(typeof data?.error === 'string' ? data.error : `Couldn't unmatch (error ${res.status}).`);
        return;
      }
      setConfirmUnmatch(false);
      setUnmatchNotice({
        text: data?.bank_transaction_id
          ? 'Unmatched. The bank transaction is now its own row, and future syncs will leave this entry alone.'
          : 'Unmatched. This account is no longer syncing, so no separate bank row was added.',
        bankId: data?.bank_transaction_id ?? null,
      });
      load();
    } catch {
      setUnmatchError("Couldn't unmatch. Check your connection and try again.");
    } finally {
  ...
  const bankMatched = !!transaction.teller_transaction_id && transaction.source !== 'bank_sync';
    ...
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {bankMatched && (
                <span
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-teal-50 text-teal-700"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {bankMatched && !confirmUnmatch && (
            <button
              type="button"
              onClick={() => { setConfirmUnmatch(true); setUnmatchError(null); setUnmatchNotice(null); }}
              disabled={!!actionLoading}
              className="min-h-11 flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 disabled:opacity-50 transition"
            >
              <Unlink className="w-3.5 h-3.5" aria-hidden="true" /> Unmatch from bank
            </button>
          )}
        ...
        {confirmUnmatch && (
          <div className="mt-3 p-3 rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-700 space-y-3">
            <p>
              A bank sync linked this entry to a bank transaction with the same amount and a nearby date. If they are
              different purchases, unmatch them: this entry stays as it is, and the bank&rsquo;s transaction is added as
              its own row.
            </p>
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              >
                {actionLoading === 'unmatch' && <Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" />}
                Unmatch
              </button>
              <button
                type="button"
                onClick={() => { setConfirmUnmatch(false); setUnmatchError(null); }}
                disabled={!!actionLoading}
                className="min-h-11 px-4 rounded-lg border border-gray-200 bg-white text-gray-700 text-sm font-medium hover:bg-gray-100 disabled:opacity-50 transition"
              >
                Cancel
              </button>
            </div>
        ...
        {unmatchError && (
          <p role="alert" className="mt-3 p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">
            {unmatchError}
          </p>
        )}

        {unmatchNotice && (
          <p role="status" className="mt-3 p-3 rounded-lg bg-sky-50 border border-sky-200 text-sm text-sky-900">
            {unmatchNotice.text}
            {unmatchNotice.bankId && (
              <>
                {' '}
                <Link
                  href={`/dashboard/finance/transactions/${unmatchNotice.bankId}`}
                  className="font-medium text-sky-700 underline"
                >
                  View the bank transaction
                </Link>
              </>
            )}
          </p>
