...
import { app, auth, db } from '@/lib/firebase'; // <-- IMPORT a single, shared Firebase instance

// --- Type Definitions for Safety ---
type TaskTag = 'FITNESS' | 'CREATIVE' | 'SKILL' | 'OUTREACH' | 'LIFESTYLE' | 'MINDSET' | 'FUEL';
type ViewMode = 'WEEK' | 'THREE_DAY' | 'DAY'; // <-- NEW: View Mode Type
...
// The global __app_id was not being set, causing potential issues.
const appId = process.env.NEXT_PUBLIC_APP_ID || 'default-centenarian-app';
const initialAuthToken: string | null = null; // This was also likely unset. We will rely on anonymous sign-in.
...
const StrategyTemplate: React.FC<StrategyTemplateProps> = ({ 
    ...
    const [viewMode, setViewMode] = useState<ViewMode>('WEEK'); // <-- NEW: View Mode State
        ...
        if (db && userId) {
            ...
            const unsubscribe = onSnapshot(docRef, (docSnap) => {
                if (docSnap.exists()) {
                    ...
                    const today = new Date();
                    // CRITICAL FIX 2: Use the correctly defined weekRanges
    ...
    }, [db, userId, startDate, weeksToShow, JSON.stringify(templatePlan), weekRanges]);
    ...
    const tasksByDay = useMemo(() => {
        ...
        // Remove keys if the resulting array is empty (e.g., if a day has no tasks)
        Object.keys(grouped).forEach(key => {
            if (grouped[key].length === 0) {
                // If in WEEK mode, filter out days with no tasks to keep the view clean
                if (viewMode === 'WEEK' && filteredTasks.filter(t => getDayName(new Date(t.date).getDay()) === key).length === 0) {
                    // Check if this date is actually *in* the current week before deleting
                    const dateStr = datesToDisplay.find(ds => getDayName(new Date(ds).getDay()) === key);
                    if (dateStr && filteredTasks.filter(t => t.date === dateStr).length === 0) {
                       delete grouped[key];
                    }
                }
            }
        ...
        return grouped;
    }, [filteredTasks, datesToDisplay, viewMode]);
